...
class Db {
    ...
    /**
     * Name for database-table with original texts.
     *
     * @var string
     */
	...
	/**
	 * Name for database-table with objects which are using original texts.
	 *
	 * @var string
	 */
    ...
    /**
     * Name for database-table with translations.
     *
     * @var string
     */
    ...
    private function __construct() {
        ...
        $this->table_name_originals = $this->get_wpdb_prefix() . 'easy_language_originals';

	    // set the table-name for objects which are using original-texts.
	    $this->table_name_originals_objects = $this->get_wpdb_prefix() . 'easy_language_originals_objects';

        // set the table-name for translations.
        $this->table_name_translations = $this->get_wpdb_prefix() . 'easy_language_translations';
    }
    ...
    public function create_table(): void {
        ...
        $sql = "CREATE TABLE $this->table_name_originals (
	    ...
	    $sql = "CREATE TABLE $this->table_name_originals_objects (
        ...
        $sql = "CREATE TABLE $this->table_name_translations (
            `oid` mediumint(9) NOT NULL,
            `time` datetime DEFAULT '0000-00-00 00:00:00' NOT NULL,
    ...
    public function delete_table(): void {
        $sql = 'DROP TABLE IF EXISTS '.$this->table_name_originals;
        $this->wpdb->query($sql);
        $sql = 'DROP TABLE IF EXISTS '.$this->table_name_originals_objects;
        $this->wpdb->query($sql);
        $sql = 'DROP TABLE IF EXISTS '.$this->table_name_translations;
        $this->wpdb->query($sql);
    }
    ...
    public function add( string $text, string $source_language, string $field ): false|Text {
        ...
        $this->wpdb->insert( $this->table_name_originals, $query);
    ...
    public function get_entries( array $filter = array() ): array {
        ...
        if( !empty($filter) ) {
            ...
            if( !empty($filter['object_id']) ) {
	            $sql_join[$this->table_name_originals_objects] = ' INNER JOIN '.$this->table_name_originals_objects.' oo ON oo.oid = o.id';
	    ...
	    $sql = 'SELECT `id`, `original`, `lang`%1$s FROM '.$this->table_name_originals .' AS o';
        ...
        $prepared_sql = $this->wpdb->prepare($sql, $vars);
    ...
    public function reset_translations(): void {
        $sql = 'TRUNCATE TABLE '.$this->table_name_translations;
        $this->wpdb->query($sql);
    }
