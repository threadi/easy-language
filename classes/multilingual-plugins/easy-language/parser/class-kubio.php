<?php
/**
 * File for handling Kubio pagebuilder for simplifications.
 *
...
/**
 * Handler for parsing Kubio-blocks.
 */
class Kubio extends Parser_Base implements Parser {
	/**
	 * Internal name of the parser.
	 *
	 * @var string
	 */
	protected string $name = 'Kubio';
	 ...
	 * @var ?Kubio
	 */
	private static ?Kubio $instance = null;
	 ...
	 */
	public static function get_instance(): Kubio {
	...
	private function get_flow_text_blocks(): array {
		 ...
		 */
		return apply_filters( 'easy_language_kubio_blocks', $blocks );
	}
	...
	public function is_object_using_pagebuilder( Post_Object $post_object ): bool {
		return function_exists( 'has_blocks' ) && has_blocks( $post_object->get_content() ) && ! empty( get_post_meta( $post_object->get_id(), 'saved_in_kubio', true ) );
	}
	...
	public function is_active(): bool {
		return Helper::is_plugin_active( 'kubio/plugin.php' );
	}
}
