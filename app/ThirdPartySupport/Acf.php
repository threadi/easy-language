<?php
/**
 * File for initializing the ACF-support.
 *
 ...
 */
class Acf extends Base implements ThirdPartySupport_Base {
	...
	public function add_meta_fields_to_simplification( Post_Object $object, int $post_id, string $source_language ): void {
		...
		foreach ( $fields as $key => $content ) {
			...
			if ( ! $original_meta_obj instanceof Text ) {
				...
				$original_meta_obj = $db->add( $content, $source_language, 'acf_meta_' . $key, $field_types[ $field_array['type'] ] );
				if ( ! $original_meta_obj instanceof Text ) {
					continue;
				}
			}
	...
	public function replace_meta_field_texts( Text $text, string $target_language, int $object_id, array $simplification_objects ): void {
		foreach ( $simplification_objects as $simplification_object ) {
			update_field( str_replace( 'acf_meta_', '', $simplification_object['field'] ), $text->get_simplification( $target_language ), $object_id );
		}
	}
