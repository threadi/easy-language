 ...
 */
function easy_language_settings_add_api_logs_tab( $tab ): void
{
	// check active tab.
	$activeClass = '';
	if( 'api_logs' === $tab ) $activeClass = ' nav-tab-active';

	// output tab.
	echo '<a href="'.esc_url(Helper::get_settings_page_url()).'&tab=api_logs" class="nav-tab'.esc_attr($activeClass).'">'.__('API Logs', 'easy-language').'</a>';
}
...
function easy_language_admin_add_menu_content_api_logs(): void {
	...
	<div class="wrap">
		...
		<h2><?php _e('API Logs', 'easy-language'); ?></h2>
		<?php $log->display(); ?>
	</div>
	<?php
}
